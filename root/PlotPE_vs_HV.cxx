...
////////////////////////////////////////////////////////////////////////////////
float PlotPhotoElectronPeaks_vs_HV(const char *SN, const char *root_dir, const int argASIC, const int argCH) {
  gStyle->SetOptFit();
  ...
  Float_t HV, OCTprob, dOCTprob, MG_Ampl, dMG_Ampl;

  TTree* tree;
  if (gDirectory->Get("PEvsHV")){
    tree = (TTree*)gDirectory->Get("PEvsHV");
    ...
    tree->SetBranchAddress("OCTprob", &OCTprob);
    tree->SetBranchAddress("dOCTprob", &dOCTprob);
    tree->SetBranchAddress("MG_Ampl", &MG_Ampl);
    tree->SetBranchAddress("dMG_Ampl", &dMG_Ampl);
  }
  else {
    printf("\033[31mCan't find TTree PEvsHV in %s . . . exiting!\033[0m\n", root_dir);
    exit(-1);
  }
  ...
  Double_t hv[NumEnt], OV[NumEnt];
  Double_t Chi[NumEnt], dChi[NumEnt];
  Double_t MGamp[NumEnt], dMGamp[NumEnt];
...
//    cout << "Gain " << i << ": " << Gain[i] << " +/- " << dGain[i];
    ...
    hv[i] = (Double_t)HV;
    Chi[i] = (Double_t)OCTprob;
    dChi[i] = (Double_t)dOCTprob;
    MGamp[i] = (Double_t)MG_Ampl/150000.;
    dMGamp[i] = (Double_t)dMG_Ampl/150000;
  }
...
cout << "\n\n\033[36mX-intersect average: \033[0m" << x0avg << "\n\033[36mY-intersect average: \033[0m" << y0avg << "\n\n";
  for (int i=0; i<NumEnt; i++) OV[i] = hv[i] - x0avg;
  ...
  mgA->SetMinimum(0);
  mgA->SetMaximum(200);
  mgA->GetYaxis()->SetTitle("Mean Peak Height (ADC Counts)");
  mgA->GetYaxis()->SetLabelSize(0.025);
  mgA->GetYaxis()->SetTitleOffset(1.25);
  mgA->GetXaxis()->SetTitle("MPPC Reverse-Biased Potential (Volts)");
  ...
  char plotName[50];
  sprintf(plotName, "data/%s/plots/ch%d/PEvsHV.pdf", SN, argCH);
...
////////////////////////////////////////////////////////////////////////////////
  ...
  char lin1dof[50];
  sprintf(lin1dof, "[0]*TMath::Sqrt(x-69.1971)");
  TF1 *SlopeFit = new TF1("SlopeFit", lin1dof);

  grWidth1->SetMarkerColor(2); grWidth1->SetTitle("1 PE"); grWidth1->SetMarkerStyle(20);;
  grWidth1->Fit("SlopeFit", "", "SAMES");
  grWidth1->GetFunction("SlopeFit")->SetLineColor(2);
  mgB->Add(grWidth1);

  grWidth2->SetMarkerColor(3); grWidth2->SetTitle("2 PE"); grWidth2->SetMarkerStyle(20);
  grWidth2->Fit("SlopeFit", "", "SAMES");
  grWidth2->GetFunction("SlopeFit")->SetLineColor(3);
  mgB->Add(grWidth2);

  grWidth3->SetMarkerColor(4); grWidth3->SetTitle("3 PE"); grWidth3->SetMarkerStyle(20);
  grWidth3->Fit("SlopeFit", "", "SAMES");
  grWidth3->GetFunction("SlopeFit")->SetLineColor(4);
  mgB->Add(grWidth3);

  grWidth4->SetMarkerColor(6); grWidth4->SetTitle("4 PE"); grWidth4->SetMarkerStyle(20);
  grWidth4->Fit("SlopeFit", "", "SAMES");
  grWidth4->GetFunction("SlopeFit")->SetLineColor(6);
  mgB->Add(grWidth4);

  grWidth5->SetMarkerColor(kOrange-3); grWidth5->SetTitle("5 PE"); grWidth5->SetMarkerStyle(20);
  grWidth5->Fit("SlopeFit","FS", "SAMES");
  grWidth5->GetFunction("SlopeFit")->SetLineColor(kOrange-3);
  mgB->Add(grWidth5);
  mgB->Draw("ap");
  ...
  mgB->SetMinimum(2);
  mgB->SetMaximum(13);
  mgB->GetYaxis()->SetTitle("Width of Gaussian Fit (ADC Counts)");
  mgB->GetYaxis()->SetLabelSize(0.025);
  mgB->GetYaxis()->SetTitleOffset(1.25);
  mgB->GetXaxis()->SetTitle("MPPC Reverse-Biased Potential (Volts)");
...
//  legB->AddEntry(grWidth5, "5 PE", "lp");
  ...
  sprintf(plotName, "data/%s/plots/ch%d/PEpeakWidths.pdf", SN, argCH);
  c1->Print(plotName);
...
////////////////////////////////////////////////////////////////////////////////
  ...
  slopeGr->SetMaximum(100);
  slopeGr->GetYaxis()->SetTitle("Slope (ADC/V)");
  ...
  sprintf(plotName, "data/%s/plots/ch%d/Slope_vs_No_of_PE.pdf", SN, argCH);
  c1->Print(plotName);
...
////////////////////////////////////////////////////////////////////////////////
  ...
  sprintf(plotName, "data/%s/plots/ch%d/Gain_vs_No_of_PE.pdf", SN, argCH);
  c1->Print(plotName);
...
////////////////////////////////////////////////////////////////////////////////
  ...
  TGraphErrors *OCTprobGr = new TGraphErrors(NumEnt, hv, Chi, 0, dChi);

  OCTprobGr->SetMarkerColor(1);
  OCTprobGr->SetTitle("Optical Cross-talk Probability vs. HV");
  OCTprobGr->SetMarkerStyle(20);

  OCTprobGr->Fit("pol1","FS", "SAMES");
  OCTprobGr->GetFunction("pol1")->SetLineColor(1);
//  double slpPerPEyInt = f0->GetParameter(0); double dslpPerPEyInt = f0->GetParError(0);
//  double slpPerPEslpe = f0->GetParameter(1); double dslpPerPEslpe = f0->GetParError(1);

  OCTprobGr->Draw("ap");
  OCTprobGr->SetMinimum(0);
  OCTprobGr->SetMaximum(0.4);
  OCTprobGr->GetYaxis()->SetTitle("Optical Cross-talk Probability");
  OCTprobGr->GetYaxis()->SetTitleOffset(1.52);
  OCTprobGr->GetXaxis()->SetTitle("HV (V)");
//  OCTprobGr->GetXaxis()->SetRangeUser(0.,6.); OCTprobGr->GetXaxis()->SetNdivisions(6);
  OCTprobGr->Draw("ap");

  c1->Update();
  TPaveStats *stsOCTprob = (TPaveStats*)OCTprobGr->GetListOfFunctions()->FindObject("stats");
  stsOCTprob->SetTextColor(1);
  stsOCTprob->SetX1NDC(0.13); stsOCTprob->SetX2NDC(0.33);
  stsOCTprob->SetY1NDC(0.82); stsOCTprob->SetY2NDC(0.89);
  c1->Modified();

  sprintf(plotName, "data/%s/plots/ch%d/OCTprob_vs_HV.pdf", SN, argCH);
  c1->Print(plotName);
//  c1->Write("Slope_vs_No_of_PE");


////////////////////////////////////////////////////////////////////////////////
/////////////////////// MultiGauss Amplitude vs. HV /////////////////////////
////////////////////////////////////////////////////////////////////////////////
  c1->Clear();

  TGraphErrors *MGamplGr = new TGraphErrors(NumEnt, hv, MGamp, 0, dMGamp);

  MGamplGr->SetMarkerColor(1);
  MGamplGr->SetTitle("Normalized g(x) Amplitude vs. HV");
  MGamplGr->SetMarkerStyle(20);


  char pwrLaw[50];
  sprintf(pwrLaw, "[0]/TMath::Power(x-[1],[2])");
  TF1 *PowerLaw = new TF1("PowerLaw", pwrLaw);
  PowerLaw->SetParameters( 20000, 70, 1.15);
  MGamplGr->Fit("PowerLaw","FS", "SAMES");
  MGamplGr->GetFunction("PowerLaw")->SetLineColor(1);
//  double slpPerPEyInt = f0->GetParameter(0); double dslpPerPEyInt = f0->GetParError(0);
//  double slpPerPEslpe = f0->GetParameter(1); double dslpPerPEslpe = f0->GetParError(1);

  MGamplGr->Draw("ap");
  MGamplGr->SetMinimum(0);
  MGamplGr->SetMaximum(1);
  MGamplGr->GetYaxis()->SetTitle("Normalized g(x) Amplitude (A/nEvts)");
  MGamplGr->GetYaxis()->SetTitleOffset(1.4);
  MGamplGr->GetXaxis()->SetTitle("HV (V)");
//  MGamplGr->GetXaxis()->SetRangeUser(0.,6.); MGamplGr->GetXaxis()->SetNdivisions(6);
  MGamplGr->Draw("ap");

  c1->Update();
  TPaveStats *stsMGamplGr = (TPaveStats*)MGamplGr->GetListOfFunctions()->FindObject("stats");
  stsMGamplGr->SetTextColor(1);
  stsMGamplGr->SetX1NDC(0.13); stsMGamplGr->SetX2NDC(0.33);
  stsMGamplGr->SetY1NDC(0.82); stsMGamplGr->SetY2NDC(0.89);
  c1->Modified();

  sprintf(plotName, "data/%s/plots/ch%d/MGampl_vs_HV.pdf", SN, argCH);
  c1->Print(plotName);
//  c1->Write("Slope_vs_No_of_PE");
...
////////////////////////////////////////////////////////////////////////////////
  ...
  mgD->SetMinimum(0);
  mgD->SetMaximum(700);
  mgD->GetYaxis()->SetTitle("Mean Peak Height (ADC Counts)");
  mgD->GetYaxis()->SetLabelSize(0.025);
  mgD->GetYaxis()->SetTitleOffset(1.45);
  mgD->GetXaxis()->SetLimits(69,72);
  mgD->GetXaxis()->SetTitle("MPPC Reverse-Biased Potential (Volts)");
  ...
  Float_t HVsat40, SatPoint = 600.;
  HVsat40 = (SatPoint-lYmin)*(grXmax-grXmin)/(lYmax-lYmin) + grXmin;

  // draw exclusion zone
  TBox *yExcl = new TBox(grXmin, SatPoint, grXmax,700);//"no border"
  ...
  legD->Draw();
  sprintf(plotName, "data/%s/plots/ch%d/PredictedPEpeaks_vs_HV.pdf", SN, argCH);
  c1->Print(plotName);
