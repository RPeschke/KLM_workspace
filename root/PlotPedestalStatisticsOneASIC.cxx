...
//  Int_t Wctime, ASIC, PeakTime[16];
  ...
  TH2I* hPeds = new TH2I("hPeds", plotTitle, 101,-50,50, 512,0,512);
  hPeds->GetXaxis()->SetTitle("Wilk. ADC Counts");
  hPeds->GetYaxis()->SetTitle("Window #");
//
    ...
    for (int ch=0; ch<15; ch++){
      for (int grp=0; grp<4; grp++){
        for (int samp=0; samp<32; samp++){
          ...
          hPeds->Fill(Sample[ch][samp+32*grp], AddNum+grp);
          //(ch==15) ? h->Fill(AddNum+grp,samp,Sample[ch][samp+32*grp]-2050) : h->Fill(AddNum+grp,samp,Sample[ch][samp+32*grp]-3400);
          //(ch==15) ? hCalib->Fill(Sample[15][samp+32*grp]) : hPeds->Fill(Sample[ch][samp+32*grp]);
        }
      }
    }
  }
