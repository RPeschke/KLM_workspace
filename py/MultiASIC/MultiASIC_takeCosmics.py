...
#    for hvOffset in range(30, 0, -5):
    ...
    for ASIC in range(10):
        if (2**ASIC & ASICmask) > 0:
            for ch in range (15):
                if (ch==testCH):
                    cmdASIC_HV += hex( int('C',16)*(2**28) | ASIC*(2**20) | (ch)*(2**16)   | hvHigh[ASIC][ch]   ).split('x')[1]+"AE000100"
    ...
    ROOT.MakeCalibTTrees("temp/waveformSamples.txt", root_file, "CosmicData", testCH)#, 1)#ascii,file,dir,CH,(opt bool)spcSvr=0
    ...
    for ASIC in range(10):
        if (2**ASIC & ASICmask) > 0:
            actHV =  floatRawHV  -  5*(hvHigh[ASIC][testCH])/255.
            dirName = "PEdata/ASIC%d/ch%d" % (ASIC, testCH)
            rootTree = "ch%d_cosmics" % testCH
