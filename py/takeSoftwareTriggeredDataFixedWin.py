...
usageMSG="Usage:\n"+\
"sudo ./py/takeSoftwareTriggeredDataFixedWin.py <S/N> <HV> <ASICmask> <Window> <numEvts>\n"+\
"Where:\n"+\
    "<S/N>          = KLMS_0XXX\n"+\
    "<HV>           = (e.g.) 74p47\n"+\
    "<ASICmask>     = (e.g.) 0000000001\n"+\
    "<Window>       = int in range 512\n"+\
    "<numEvts>      = (e.g.) 50000\n"
...
print "./py/takeSoftwareTriggeredDataFixedWin.py %s %s %s %d %d" % (SN,rawHV,sys.argv[3],Window,NumEvts)
...
trgTyp      = 0     ## '0': use trigger generated by HW or SW or anything
#trgTyp      = 1     ## '1': only use trigger generated by internal trig dec logic
...
cmd_runConfig += hex(int('AF3E0000',16) | fxdWin*(2**15) | fxdWin*Window        ).split('x')[1] +"AE000100"#OUTPUT_REGISTERS(62)
cmd_runConfig += "AF250004"+"AE000100"     #--internal_OUTPUT_REGISTERS(37)(2 downto 0) usuage (CMDREG_USE_EXTRIG):
                                           #"100/0x4": use ex trig and read from fixed asic
	                                       #"101/0x5": use ex trig and increment asic number after each trigger
	                                       #"110/0x6": use internal counter trig and read from fixed asic
	                                       #"111/0x7": use internal counter trig and increment asic number after each trigger
cmd_runConfig += hex(int('AF360000',16) | winOffset ).split('x')[1] +"AE000100"#set win offset
cmd_runConfig += hex(int('AF330000',16) | ASICmask  ).split('x')[1] +"AE000100"#set asic number
cmd_runConfig += hex(int('AF260000',16) | opMode*(2**12) | 2**7 + 2**6 + 2**5 + 2**4   ).split('x')[1] +"AE000100"#CMDREG_PedDemuxFifoOutputSelect(13 downto 12)-->either wavfifo or pedfifo,CMDREG_PedSubCalcMode(10 downto 7)-->currently only using bit 7: 1 for peaks 2 for troughs, sample offset is 3400 o6 600 respectively
cmd_runConfig += hex(int('AF270000',16) | trgTyp*(2**15) | ASICmask        ).split('x')[1] +"AE000100"
...
print "writting in temp/waveformSamples.txt"
